...
#'    Assumption: No individual columns are provided for function Ttest_chi_df().In such case a normal one on one t-test or chi would be better.
#' @param For Ttest_chi_df()----->data,type_of_test,User_BinaryTarget(optional),User_Variable(optional)
#' @return  NULL
#' @examples Ttest_chi_df(df,"chi",c(1,2),c(3,4)),Ttest_chi_df(df,"ttest",filename="Apple")
#' @export Ttest_chi_df
#' @import xlsx



Ttest_chi_df<-function(data,type_of_test,User_BinaryTarget=NULL,User_Variable=NULL,filename=NULL){
